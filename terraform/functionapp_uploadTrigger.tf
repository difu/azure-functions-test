...
resource "azurerm_function_app" "linuxfunction" {
  ...
  app_settings = {
    ...
    "WEBSITE_RUN_FROM_PACKAGE" = "https://${azurerm_storage_account.storageacct.name}.blob.core.windows.net/function-upload-code/${azurerm_storage_blob.function_upload_trigger_code_blob.name}"
