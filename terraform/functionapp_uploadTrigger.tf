...
module "uploadTriggerFunction" {
  source = "./mod_function"
  resource_group = {
    name = azurerm_resource_group.functiontest.name
    id = azurerm_resource_group.functiontest.id
    location = azurerm_resource_group.functiontest.location
  }
  function_name          = "BlobUploadTrigger"
  instrumentation_key    = azurerm_application_insights.ai_function_app.instrumentation_key
  storage_account = {
    name = azurerm_storage_account.storageacct.name
    id = azurerm_storage_account.storageacct.id
    primary_access_key = azurerm_storage_account.storageacct.primary_access_key
  }
  storage_container_name = azurerm_storage_container.function_upload_code.name
